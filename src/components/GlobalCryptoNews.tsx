    ...
    const fetchNews = async () => {
      ...
      try {
        ...
        setNews([
          {
            id: "1",
            headline: "Bitcoin Halving Impact Outperforming Previous Cycles",
            category: "Bitcoin",
            summary:
              "On-chain analytics suggest the latest BTC halving is absorbing liquid supply faster than the 2020 cycle, pointing to a potential upcoming supply shock on centralized exchanges.",
            impactScore: 9,
            timeAgo: "15m ago",
            sentiment: "bullish",
          },
          {
            id: "2",
            headline:
              "Solana Network Upgrades Yield Record Transaction Throughput",
            category: "Solana",
            summary:
              "The latest validator client update has pushed the network's theoretical TPS to new highs, bringing institutional confidence to decentralized exchanges operating on the network.",
            impactScore: 8,
            timeAgo: "30m ago",
            sentiment: "bullish",
          },
          {
            id: "3",
            headline: "Ethereum Layer 2s Dominate Total Value Locked",
            category: "Ethereum",
            summary:
              "Arbitrum and Base have seen explosive growth in the past 24 hours, collectively holding over $14B in TVL, driven by new decentralized perpetual exchanges.",
            impactScore: 7,
            timeAgo: "1h ago",
            sentiment: "bullish",
          },
          {
            id: "4",
            headline: "XRP Ledger Enhances Cross-Border Settlement Corridors",
            category: "XRP",
            summary:
              "New banking partnerships in Southeast Asia demonstrate the ledger's ongoing maturation and viability for bridging institutional liquidity.",
            impactScore: 6,
            timeAgo: "2h ago",
            sentiment: "bullish",
          },
          {
            id: "5",
            headline: "Global Regulators Coordinate on Stablecoin Framework",
            category: "Regulatory",
            summary:
              "Following recent closed-door meetings in Geneva, leading financial authorities have signaled a unified approach for classifying fiat-backed stablecoins to ensure systemic safety.",
            impactScore: 8,
            timeAgo: "3h ago",
            sentiment: "neutral",
          },
      ...
      <div className="flex items-center justify-between pb-2 border-b border-[#2A2E39]">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h3 className="text-[14px] font-black uppercase tracking-widest text-blue-400 text-shadow-glow">
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-4 relative z-10">
            {news.map((item) => {
              ...
              } else if (item.category === "Regulatory")
                catColor =
                  "bg-orange-500/10 text-orange-400 border-orange-500/30";
              else if (item.category === "DeFi")
                catColor =
                  "bg-purple-500/10 text-purple-400 border-purple-500/30";
              else if (item.category === "Web3")
                catColor = "bg-pink-500/10 text-pink-400 border-pink-500/30";
              ...
              const accentColor = isBullish
                ...
                : isBearish
                  ? "#FF4D4D"
                  : "#38bdf8"; // teal-400 is not standard, let's use tailwind sky-400 or just explicit hex
              ...
              if (brandColor) {
                ...
                boxClasses = isBullish
                  ? "bg-blue-500/[0.03] border-blue-500/20 hover:border-blue-500/50 shadow-[0_0_15px_rgba(0,255,136,0.05)]"
                  : isBearish
                    ? "bg-red-500/[0.03] border-red-500/20 hover:border-red-500/50 shadow-[0_0_15px_rgba(255,77,77,0.05)]"
              ...
              return (
                <div
                  key={item.id}
                  className={`flex flex-col gap-3 p-5 rounded-xl transition-all duration-300 border ${boxClasses}`}
                  style={inlineStyles}
                >
                  <div className="flex justify-between items-start mb-1">
                    <span
                      className={`flex items-center gap-1.5 text-[9px] uppercase tracking-widest font-bold px-2 py-1 rounded border shadow-sm ${catColor}`}
                    >
                      {Logo} {item.category}
                    </span>
                    <div className="flex items-center gap-4">
                      ...
                      <span className="flex items-center gap-1 text-[10px] uppercase font-bold text-[#787B86] bg-[#131722] px-2 py-0.5 rounded">
                        Impact{" "}
                        <span
                          className={`font-mono font-black ${item.impactScore >= 8 ? "text-red-400" : "text-teal-400"}`}
                        >
                          {item.impactScore}
                        </span>
                      </span>
                  ...
                  <h4 className="text-[15px] font-black text-white leading-tight mt-1 group-hover:text-teal-400 transition-colors">
                    {item.headline}
                  </h4>
                  <p className="text-[13px] text-[#787B86] leading-relaxed font-sans">
                    {item.summary}
                  </p>
                </div>
              );
            ...
            {error && (
              <div className="text-[10px] uppercase font-mono text-center text-red-400/50 flex justify-center items-center gap-1 mt-4">
                <AlertCircle className="w-3 h-3" /> Live feed disconnected.
                Displaying cached intelligence.
              </div>
            )}
          </div>
        )}
