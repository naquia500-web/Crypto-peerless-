      ...
      <div className="bg-[#131722] shadow-lg border border-[#2A2E39] p-6 rounded-xl flex flex-col gap-5">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-1 flex flex-col gap-2">
            ...
            <div className="flex gap-4 mt-3">
              <button 
                ...
                disabled={hasVoted}
                className={`flex-1 py-2.5 rounded-lg border border-blue-500/30 text-blue-600 transition-all text-[11px] uppercase font-bold tracking-widest flex justify-center items-center gap-2 ${hasVoted ? 'opacity-50 cursor-not-allowed bg-[#1E222D]' : 'hover:bg-blue-500/10 hover:shadow-[0_0_15px_rgba(0,255,136,0.2)]'}`}
              >
              ...
              <button 
                ...
                disabled={hasVoted}
                className={`flex-1 py-2.5 rounded-lg border border-red-500/30 text-red-500 transition-all text-[11px] uppercase font-bold tracking-widest flex justify-center items-center gap-2 ${hasVoted ? 'opacity-50 cursor-not-allowed bg-[#1E222D]' : 'hover:bg-red-500/10 hover:shadow-[0_0_15px_rgba(255,77,77,0.2)]'}`}
              >
          ...
          <div className="flex-1 flex flex-col gap-2 p-4 bg-[#1E222D] rounded-xl border border-[#2A2E39]">
             <h4 className="text-[11px] font-bold uppercase tracking-widest text-[#787B86] mb-2">Next Month Price Prediction</h4>
             <div className="space-y-3">
               <div>
                  ...
                  <div className="w-full h-1.5 bg-[#2A2E39] rounded-full overflow-hidden"><div className="h-full bg-blue-500 w-[68%]"></div></div>
               </div>
               <div>
                  <div className="flex justify-between text-xs font-mono text-[#787B86] mb-1">
                    <span>$70k - $85k</span>
                  ...
                  <div className="w-full h-1.5 bg-[#2A2E39] rounded-full overflow-hidden"><div className="h-full bg-yellow-500 w-[22%]"></div></div>
               </div>
               <div>
                  <div className="flex justify-between text-xs font-mono text-[#787B86] mb-1">
                    <span>Below $70k</span>
                  ...
                  <div className="w-full h-1.5 bg-[#2A2E39] rounded-full overflow-hidden"><div className="h-full bg-red-400 w-[10%]"></div></div>
               </div>
             </div>
        ...
        <div className="mt-4 flex flex-col gap-4 border-t border-[#2A2E39] pt-6">
          {loadingPosts ? (
            ...
            posts.map((post) => (
              <div key={post.id} className="flex gap-4 p-4 bg-[#1E222D] rounded-xl border border-[#2A2E39] hover:border-teal-500/30 transition-all cursor-pointer group">
                {post.avatar_url ? (
                  <img src={post.avatar_url} alt={post.username} className="w-10 h-10 rounded-full shrink-0 border border-teal-500/40 bg-[#131722]" />
                ) : (
                ...
                <div className="flex flex-col gap-2 w-full">
                  ...
                  {post.post_image && (
                    <div className="mt-2 w-full max-h-48 overflow-hidden rounded-lg border border-[#2A2E39] opacity-80 group-hover:opacity-100 transition-opacity">
                  ...
                  <div className="flex items-center gap-6 mt-3 text-[#787B86]">
                    <button className="flex items-center gap-1.5 hover:text-blue-600 transition-colors group-hover:text-[#787B86]"><TrendingUp className="w-4 h-4" /> <span className="text-xs font-mono font-bold text-blue-600">{post.likes}</span></button>
