...
export function AssetYield() {
  ...
  const INITIAL_YIELD = [
    ...
    },
    {
      rank: 3,
      provider: "Bybit",
      type: "Earn (Locked), Earn (Flexi), Staking",
      baseApy: 10.0,
      category: "CeFi",
    },
    {
    ...
    },
    {
      rank: 8,
      provider: "Gate.io",
      type: "Earn (Locked), Staking, Earn (Flexi)",
      baseApy: 10.3,
      category: "CeFi",
    },
  ];

  const [liveApy, setLiveApy] = useState<
    Record<number, { apy: number; status: "up" | "down" | "neutral" }>
  >({});
  ...
  useEffect(() => {
    ...
    const initial: Record<
      number,
      { apy: number; status: "up" | "down" | "neutral" }
    > = {};
      ...
      ws.onmessage = (event) => {
        ...
        if (Math.random() > 0.8) {
          setLiveApy((prev) => {
            ...
            INITIAL_YIELD.forEach((y) => {
              if (Math.random() > 0.7) {
                // 30% chance for a specific row to update
                hasChanges = true;
                const jitter = Math.random() * 0.04 - 0.02; // fluctuate by -0.02% to +0.02%
                let newApy = Math.max(
                  0.01,
                  (updated[y.rank]?.apy || y.baseApy) + jitter,
                );
      ...
      <div className="flex items-center justify-between pb-2 border-b border-[#2A2E39]">
        ...
        <div className="flex gap-2">
          {["CeFi", "DeFi", "All"].map((tab, idx) => (
            <button
              key={tab}
              className={`text-[9px] font-bold uppercase tracking-widest px-3 py-1 rounded transition-colors ${idx === 2 ? "bg-blue-100 text-blue-700" : "bg-[#2A2E39] text-[#787B86] hover:bg-slate-200 hover:text-[#B2B5BE]"}`}
            >
              {tab}
            </button>
          ))}
        ...
        <table className="w-full text-left border-collapse min-w-[700px]">
          ...
          <tbody>
            {INITIAL_YIELD.map((row) => {
              const current = liveApy[row.rank] || {
                apy: row.baseApy,
                status: "neutral",
              };
              const colorClass =
                current.status === "up"
                  ...
                  : current.status === "down"
                    ...
                    : "text-blue-400 bg-blue-500/10 border-transparent";

              return (
                <tr
                  key={row.rank}
                  className="border-b border-[#2A2E39] hover:bg-[#1E222D] transition-colors group"
                >
                  ...
                  <td className="px-4 py-4">
                    <span className="text-xs font-bold flex items-center gap-2 text-[#B2B5BE]">
                      <img
                        src={
                          LOGOS[row.provider] ||
                          `https://ui-avatars.com/api/?name=${row.provider.replace(/ /g, "+")}&background=f8fafc&color=333&rounded=true&font-size=0.4`
                        }
                        alt={row.provider}
                  ...
                  <td className="px-4 py-4">
                    <span className="text-[11px] text-[#787B86] tracking-wide">
                      {row.type}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-4 text-right">
                    <span className="text-[10px] font-bold uppercase tracking-widest text-[#787B86] bg-[#2A2E39] border border-[#2A2E39] px-2 py-0.5 rounded-sm">
                      {row.category}
                    </span>
                  </td>
                </tr>
