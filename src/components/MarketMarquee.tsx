...
export function MarketMarquee() {
  ...
  useEffect(() => {
    ...
    const streams = INITIAL_SYMBOLS.map(
      (s) => `${s.symbol.toLowerCase()}@ticker`,
    ).join("/");
  ...
  return (
    <div className="border-b border-[#2A2E39] bg-[#131722] overflow-hidden whitespace-nowrap h-12 flex items-center">
      <div
        className="animate-marquee inline-flex gap-8 px-4"
      ...
      >
        {[...INITIAL_SYMBOLS, ...INITIAL_SYMBOLS].map((asset, i) => {
          ...
          return (
            ...
            >
              <span className="text-xs font-bold uppercase tracking-widest text-[#787B86]">
                {asset.name}
              </span>
              {data ? (
                <>
                  <span className="data-value text-white font-mono font-bold">
                    ${data.price}
                  </span>
                  <span
                    className={`inline-flex items-center text-[10px] font-mono font-bold ${isPositive ? "text-green-500" : "text-red-400"}`}
                  >
                    {isPositive ? "+" : ""}
                    {parseFloat(data.changePercent).toFixed(2)}%
                  </span>
                </>
              ) : (
                <span className="text-slate-400 font-mono text-xs animate-pulse">
                  Loading...
                </span>
              )}
            </div>
