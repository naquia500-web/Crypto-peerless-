    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col gap-4">
          <div className="p-6 bg-white/[0.02] border border-white/5 rounded-2xl h-full">
            ...
            <div className="flex flex-col gap-4">
              {GUIDELINES.map((rule) => {
                ...
                return (
                  <div key={rule.id} className="flex gap-4 p-4 rounded-xl bg-[#0B0E11] border border-white/5 hover:border-white/10 transition-colors">
                    <div className="mt-1">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-white/50 leading-relaxed font-sans">{rule.desc}</p>
                    </div>
                  </div>
                );
              })}
        ...
        {/* Support Form */}
        <div className="flex flex-col gap-4">
          <form onSubmit={handleSubmit} className="p-6 bg-gradient-to-br from-[#0B0E11] to-blue-900/10 border border-white/10 hover:border-blue-500/30 transition-colors rounded-2xl flex flex-col relative overflow-hidden h-full">
              ...
              <div className="flex flex-col gap-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input required type="text" placeholder="John Doe" className="bg-black/50 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white focus:border-blue-500 focus:outline-none transition-colors" />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-bold text-white/70 uppercase tracking-wider">Email</label>
                    <input required type="email" placeholder="john@example.com" className="bg-black/50 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white focus:border-blue-500 focus:outline-none transition-colors" />
                  </div>
                </div>

                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold text-white/70 uppercase tracking-wider">Issue Type</label>
                  <select className="bg-black/50 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white focus:border-blue-500 focus:outline-none transition-colors appearance-none">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <textarea required rows={4} placeholder="Describe your issue in detail..." className="bg-black/50 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white focus:border-blue-500 focus:outline-none transition-colors resize-none"></textarea>
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div 
                    className={`border-2 border-dashed rounded-xl p-4 text-center cursor-pointer transition-colors ${dragActive ? 'border-blue-500 bg-blue-500/5' : 'border-white/10 bg-black/20 hover:border-white/30'}`}
                  ...
                  >
                    ...
                    <p className="text-xs text-white/60">Click to upload or drag and drop images here</p>
                  ...
                  {selectedImages.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {selectedImages.map((file, i) => (
                        <div key={i} className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-md py-1 px-2 pr-1">
