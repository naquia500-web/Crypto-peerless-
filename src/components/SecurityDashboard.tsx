...
import { Shield, Smartphone, Key, MapPin, Activity, Loader2, Fingerprint } from 'lucide-react';
...
export function SecurityDashboard() {
  ...
  const activities = [
    { type: 'Login', device: 'Chrome / Windows', location: 'Mumbai, India', timeStr: '2 mins ago' },
    { type: 'Login', device: 'Safari / iOS', location: 'Delhi, India', timeStr: '1 hour ago' },
    { type: 'API Key', device: 'System Process', location: 'Frankfurt, DE', timeStr: 'Just now' },
    { type: 'Login', device: 'Firefox / macOS', location: 'Bangalore, India', timeStr: '5 mins ago' },
    { type: 'Session Auth', device: 'Mobile App / Android', location: 'Pune, India', timeStr: '15 mins ago' },
    { type: 'Withdrawal', device: 'Chrome / ChromeOS', location: 'Singapore', timeStr: '22 mins ago' },
    { type: 'New Device', device: 'Edge / Windows', location: 'Dubai, UAE', timeStr: '1 day ago' },
  ];
        ...
        <div className="flex items-center justify-between bg-[#1E222D] p-3 rounded-lg border border-[#2A2E39] hover:border-green-500/30 transition-colors">
          <div className="flex items-center gap-3">
            <Smartphone className="w-4 h-4 text-blue-400" />
             <span className="text-sm text-gray-300 font-medium">2-Factor Authentication</span>
          </div>
          <span className="text-xs font-bold text-green-500 flex items-center gap-1">
             <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></span> Enabled
          </span>
        </div>
        ...
        <div className="flex items-center justify-between bg-[#1E222D] p-3 rounded-lg border border-[#2A2E39] hover:border-purple-500/30 transition-colors">
          <div className="flex items-center gap-3">
            <Fingerprint className="w-4 h-4 text-purple-400" />
             <span className="text-sm text-gray-300 font-medium">Anti-Phishing Code</span>
          </div>
          <span className="text-xs font-bold text-green-500 flex items-center gap-1">
      ...
      <div className="border-t border-[#2A2E39] pt-4 mt-auto">
        ...
        <div className="flex items-start gap-3 bg-[#131722] p-2 rounded-lg group hover:bg-[#1E222D] transition-colors cursor-default">
          ...
          <div className="flex flex-col flex-1 overflow-hidden">
            <span className="text-sm text-white font-bold truncate leading-tight group-hover:text-blue-400 transition-colors">{currentActivity.type} / {currentActivity.device}</span>
            <span className="text-xs text-gray-500 truncate mt-1 font-medium">{currentActivity.location} • {currentActivity.timeStr}</span>
          </div>
