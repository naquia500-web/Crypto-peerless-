  ...
  useEffect(() => {
    ...
    const generateAlert = () => {
      ...
      if ("Notification" in window && Notification.permission === "granted") {
        new Notification(
          `Market Alert: ${coin.symbol} ${isPump ? "Surge" : "Drop"}`,
          {
      ...
      setTimeout(() => {
        setActiveAlerts((current) =>
          current.filter((a) => a.id !== newAlert.id),
        );
      }, 5000);
    };
  ...
  return (
    <div className="flex flex-col gap-6 w-full relative">
      ...
      <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-3 pointer-events-none">
        <AnimatePresence>
          {activeAlerts.map((alert) => (
            ...
            >
              <div
                className={`absolute top-0 left-0 w-1 h-full ${alert.type === "pump" ? "bg-green-500" : "bg-red-500"}`}
              ></div>

              <div className="w-10 h-10 rounded-full bg-[#1E222D] flex items-center justify-center shrink-0 p-1">
                <img
                  src={
                    getCryptoLogo(alert.symbol) ||
                    `https://ui-avatars.com/api/?name=${alert.symbol}&background=random`
                  }
                  alt={alert.symbol}
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start mb-1">
                  ...
                  <span
                    className={`text-[10px] font-bold ${alert.type === "pump" ? "text-green-400" : "text-red-400"} flex items-center gap-1`}
                  >
                    {alert.type === "pump" ? (
                ...
                <p className="text-xs text-[#787B86] leading-relaxed mb-2">
                  {alert.message}
                </p>
                <div className="text-[10px] font-mono text-white/40">
                  Current Price:{" "}
                  <span className="text-[#787B86]">{alert.price}</span>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div
          id="section-global-alerts"
          className="p-6 bg-gradient-to-br from-[#0B0E11] to-purple-900/10 border border-purple-500/20 rounded-2xl relative overflow-hidden group hover:border-purple-500/50 transition-all duration-500 flex flex-col"
        >
          ...
          <div className="flex justify-between items-start mb-6">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-purple-500/20 border border-purple-500/50 flex items-center justify-center relative">
                <Activity
                  className={`w-6 h-6 text-purple-400 ${alertsEnabled ? "animate-pulse" : ""}`}
                />
              </div>
              {alertsEnabled && (
          ...
          <p className="text-sm text-[#787B86] mb-6 min-h-[60px] leading-relaxed">
            Analyzes real-time data and news from all major crypto exchanges
            globally to send instant notifications on significant market
            movements or anomalies.
          </p>
