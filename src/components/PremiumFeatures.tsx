...
export function PremiumFeatures({ onOpenLiveCoach }: { onOpenLiveCoach: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Live Trading Coach */}
        <div id="section-live-trading" className="p-6 bg-gradient-to-br from-[#0B0E11] to-blue-900/10 border border-blue-500/20 rounded-2xl relative overflow-hidden group hover:border-blue-500/50 transition-all duration-500 flex flex-col">
          <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/10 blur-3xl rounded-full pointer-events-none group-hover:bg-blue-500/20 transition-all"></div>
          
          <div className="flex justify-between items-start mb-6 align-top">
            <div className="w-12 h-12 rounded-xl bg-blue-500/20 border border-blue-500/50 flex items-center justify-center relative">
              <Bot className="w-6 h-6 text-blue-400" />
              <span className="absolute -bottom-1 -right-1 flex h-3 w-3">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 justify-center"></span>
                <span className="relative inline-flex rounded-full h-3 w-3 bg-green-500 border-2 border-[#0B0E11]"></span>
              </span>
            </div>
            <div className="px-3 py-1 rounded-full bg-blue-500/20 border border-blue-500/30 text-[10px] uppercase font-bold text-blue-400 tracking-wider">
              Trading Coach AI
            </div>
          </div>
          ...
          <div className="mt-auto pt-4">
            <button 
              ...
              className="w-full py-3 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/30 hover:border-blue-500/60 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 group/btn"
            >
              Start Live Session <ArrowRight className="w-4 h-4 group-hover/btn:translate-x-1 transition-transform" />
            </button>
          </div>
        </div>
