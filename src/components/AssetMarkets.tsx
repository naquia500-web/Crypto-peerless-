...
export function AssetMarkets() {
  ...
  const MARKETS_DATA: Record<string, any[]> = {
    Spot: [
      ...
      },
      {
        ...
        exchange: "Binance",
        pair: "BTC/USDC",
        defaultPrice: 76592.86,
        depth: "$8.1M / $4.2M",
        volume: "$618.7M",
        volPct: "1.61%",
        liquidity: 945,
        liqPct: 80,
        url: "https://www.binance.com/en/trade/BTC_USDC",
        offset: -1.5,
      },
      {
        rank: 3,
    ...
    Perpetual: [
      {
        ...
        exchange: "Binance",
        pair: "BTC/USDT",
        defaultPrice: 76595.12,
        depth: "$89.2M / $92.4M",
        volume: "$12.4B",
        volPct: "32.1%",
        liquidity: 1450,
        liqPct: 98,
        url: "https://www.binance.com/en/futures/BTC_USDT",
        offset: 1.5,
      },
      {
        rank: 2,
        exchange: "Bybit",
        pair: "BTC/USDT",
        defaultPrice: 76593.8,
        depth: "$45.1M / $42.2M",
        volume: "$8.2B",
        volPct: "21.3%",
        liquidity: 1200,
        liqPct: 88,
        url: "https://www.bybit.com/en-US/trade/linear/BTCUSDT",
        offset: 0.1,
      },
      {
        rank: 3,
      ...
      },
      {
        ...
        exchange: "Bitget",
        pair: "BTC/USDT",
        defaultPrice: 76591.9,
        depth: "$22.4M / $25.1M",
        volume: "$4.1B",
        volPct: "10.6%",
        liquidity: 980,
        liqPct: 76,
        url: "https://www.bitget.com/mix/usdt/BTCUSDT",
        offset: -1.8,
      },
      {
        rank: 5,
    ...
    Futures: [
      {
        ...
        exchange: "Binance",
        pair: "BTC/USD COIN-M",
        defaultPrice: 76650.0,
        depth: "$18.4M / $15.2M",
        volume: "$1.8B",
        volPct: "4.7%",
        liquidity: 950,
        liqPct: 80,
        url: "https://www.binance.com/en/delivery/btc_usd",
        offset: 56.3,
      },
      {
        rank: 2,
      ...
      },
      {
        rank: 3,
        exchange: "Bybit",
        pair: "BTC/USD Inverse",
        defaultPrice: 76648.2,
        depth: "$9.5M / $11.2M",
        volume: "$800.5M",
        volPct: "2.1%",
        liquidity: 780,
        liqPct: 65,
        url: "https://www.bybit.com/en-US/trade/inverse/BTCUSD",
        offset: 54.5,
      },
    ],
  };
  ...
  const [liveData, setLiveData] = useState<
    Record<number, { price: number; status: "up" | "down" | "neutral" }>
  >({});
  ...
  useEffect(() => {
    ...
    const initial: Record<
      number,
      { price: number; status: "up" | "down" | "neutral" }
    > = {};
    ...
    const connectWs = () => {
      ...
      ws.onclose = () => {
        if (isSubscribed) setTimeout(connectWs, 5000);
      };
    };
        ...
        <div className="flex gap-2">
          {["Spot", "Perpetual", "Futures"].map((tab) => (
            <button
              ...
              className={`text-[9px] font-bold uppercase tracking-widest px-3 py-1 rounded transition-colors ${activeTab === tab ? "bg-blue-100 text-blue-700" : "bg-[#2A2E39] text-[#787B86] hover:bg-slate-200 hover:text-[#B2B5BE]"}`}
            >
              {tab}
            </button>
          ))}
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
          ...
          <tbody>
            {MARKETS_DATA[activeTab].map((row) => {
              const current = liveData[row.rank] || {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-4">
                    <span className="text-xs font-bold flex items-center gap-2 group-hover:text-blue-400 transition-colors text-[#B2B5BE]">
                      <img
                        src={
                          LOGOS[row.exchange] ||
                          `https://ui-avatars.com/api/?name=${row.exchange.replace(/ /g, "+")}&background=f8fafc&color=333&rounded=true&font-size=0.4`
                        }
                        alt={row.exchange}
                  ...
                  <td className="px-4 py-4">
                    <span className="text-[11px] font-bold uppercase tracking-widest text-[#787B86]">
                      {row.pair}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-4 text-right">
                    <span className="font-mono text-xs text-[#B2B5BE]">
                      {row.volume}
                    </span>
                  </td>
                  <td className="px-4 py-4 text-right">
