...
const INITIAL_COINS = [
  ...
  },
  {
    ...
    name: "Cosmos",
    symbol: "ATOM",
    match: "Lowest Mkt Cap",
    score: "3.9/10",
    icon: AlertCircle,
    data: generateSparkline(30, "down"),
  },
];
          ...
          INITIAL_COINS.map(async (coin) => {
            ...
            try {
              ...
              const klinesRes = await fetch(
                `https://api.binance.com/api/v3/klines?symbol=${coin.id}&interval=${interval}&limit=${limit}`,
              );
              const klinesJson = await klinesRes.json();
              if (Array.isArray(klinesJson)) {
                sparklineData = klinesJson.map((candle: any) => ({
                  val: parseFloat(candle[4]),
                }));
              }
            } catch (e) {
                ...
                setCoins((prevCoins) => {
                  ...
                  const newCoins = prevCoins.map((c) => {
                    ...
                    if (tData) {
                      const currentPrice =
                        c.price === "..."
                          ...
                          : typeof c.price === "string"
                            ? parseFloat(c.price.replace(/[^0-9.-]+/g, ""))
                            : c.price;
                      ...
                      if (
                        Math.abs(newPriceNum - currentPrice) > 0 ||
                        Math.random() > 0.5
                      ) {
                        hasChanges = true;
                        ...
                        const simulatedPrice =
                          newPriceNum +
                          newPriceNum * (Math.random() * 0.0002 - 0.0001);
        ...
        <div className="flex gap-2">
          {(["24h", "7d", "30d"] as const).map((tf) => (
            <button
              ...
              className={`text-[9px] font-bold uppercase tracking-widest px-2 py-1 flex items-center justify-center rounded transition-colors ${timeframe === tf ? "bg-[#3861FB] text-white" : "bg-[#1E222D] hover:bg-[#2A2E39] text-[#787B86] hover:text-white"}`}
            >
              {tf}
            </button>
          ))}
          <button className="text-[9px] font-bold uppercase tracking-widest bg-[#1E222D] hover:bg-[#2A2E39] px-3 py-1 rounded transition-colors ml-2">
            See More
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {coins.map((coin) => {
          ...
          return (
            <div
              key={coin.id}
              className="bg-[#131722] shadow-lg border border-[#2A2E39] p-4 rounded-xl flex flex-col relative group hover:bg-[#2A2E39] transition-colors cursor-pointer"
            >
                  ...
                  <AreaChart data={coin.data}>
                    <defs>
                      ...
                      >
                        <stop
                          offset="5%"
                          stopColor={coin.isUp ? "#00FF88" : "#FF4D4D"}
                          stopOpacity={0.3}
                        />
                        <stop
                          offset="95%"
              ...
              <div className="mt-auto flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  <img
                    src={
                      ...
                      `https://ui-avatars.com/api/?name=${coin.symbol.replace(/ /g, "+")}&background=0B0E11&color=fff&rounded=true&font-size=0.4`
                    }
                    alt={coin.symbol}
                    className="w-5 h-5 rounded-full bg-[#1E222D]"
                  />
                  <span className="font-bold text-sm">{coin.name}</span>
                  <span className="text-[10px] uppercase font-bold opacity-50 bg-[#1E222D] px-1 rounded">
                    {coin.symbol}
                  </span>
                </div>
                <div className="flex items-center gap-2 font-mono">
                  ...
                  <span
                    className={`text-[10px] font-bold ${coin.isUp ? "text-blue-400" : "text-red-400"}`}
                  >
                    {coin.change}
                  </span>
                </div>
