  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex justify-between items-center px-1">
        <h3 className="text-[11px] font-black uppercase tracking-widest opacity-80">
          Live Ticker
        </h3>
      ...
      <div className="flex flex-col gap-5">
        {MOVERS.map((asset, i) => (
          <div
            key={asset.symbol}
            className="flex justify-between items-center border-b border-[#2A2E39] pb-3 cursor-pointer hover:bg-[#2A2E39] transition-colors px-2 -mx-2 rounded"
          >
            ...
            <div className="text-right mt-1">
              <span className="block text-sm font-mono font-bold tracking-tight">
                ${asset.price}
              </span>
              <span
                className={`block text-[10px] font-mono mt-0.5 ${asset.isPos ? "text-blue-400" : "text-red-400"}`}
              >
                {asset.change}
              </span>
            </div>
