...
import { Activity, ArrowRight, ArrowDown, ArrowUp, Filter, AlertTriangle } from 'lucide-react';
    ...
    <div className="bg-[#131722] rounded-2xl border border-[#2A2E39] shadow-sm overflow-hidden flex flex-col h-[500px]">
      <div className="p-4 border-b border-[#2A2E39] flex justify-between items-center bg-[#1E222D]">
        <h2 className="text-lg font-black text-[#D1D4DC] flex items-center gap-2">
          <Activity className="w-5 h-5 text-blue-600" /> Live Trades & Whale Alerts
        </h2>
        <div className="flex gap-2">
          {(['all', 'whale', 'retail'] as const).map(f => (
            ...
            >
              {f === 'whale' ? 'Whales (>10 BTC)' : f === 'retail' ? 'Retail' : 'All'}
            </button>
          ))}
        </div>
      </div>
          ...
          <tbody>
            <AnimatePresence initial={false}>
              {filteredTrades.map(trade => (
                <motion.tr 
                  key={trade.id}
                  initial={{ opacity: 0, y: -10, backgroundColor: trade.type === 'buy' ? 'rgba(34, 197, 94, 0.1)' : 'rgba(239, 68, 68, 0.1)' }}
                  animate={{ opacity: 1, y: 0, backgroundColor: 'transparent' }}
                  className={`border-b border-[#2A2E39] text-sm font-mono transition-colors hover:bg-[#1E222D] ${
                    trade.isWhale ? 'text-orange-500 font-bold' : 'text-[#787B86]'
                  }`}
                >
                  ...
                  <td className="p-2">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-bold ${
                      ...
                      {trade.isWhale && <AlertTriangle className="w-3 h-3 ml-1 text-orange-500" />}
                    </span>
                  </td>
                  <td className="p-2 text-right">${trade.priceUSD.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                  <td className="p-2 text-right">{trade.amountBTC.toLocaleString(undefined, { minimumFractionDigits: 4, maximumFractionDigits: 4 })}</td>
                  <td className={`p-2 text-right font-black tracking-tight ${trade.isWhale ? 'text-orange-600' : 'text-white'}`}>
                    ${trade.totalUSD.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                  </td>
                </motion.tr>
              ))}
            </AnimatePresence>
            {filteredTrades.length === 0 && (
              <tr>
                <td colSpan={5} className="text-center p-8 text-[#787B86] text-sm">Waiting for live trades...</td>
              </tr>
            )}
          </tbody>
        </table>
