...
export function GlobalPrices({ currentBtcPrice = 76683.5 }: GlobalPricesProps) {
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {pairs.map((pair: any) => (
            <div
              key={pair.target}
              className="flex items-center gap-2 px-3 py-2 bg-[#1E222D] hover:bg-[#2A2E39] border border-[#2A2E39] rounded transition-colors group cursor-pointer"
            >
              <img
              ...
              <AnimatePresence mode="popLayout">
                <motion.span
                  ...
                  className="text-[11px] font-mono font-bold text-blue-400 ml-1 bg-blue-500/10 px-1 rounded"
                >
                  {pair.symbol}
                  {(currentBtcPrice * pair.currentRate).toLocaleString(
                    undefined,
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {liveTrending.map((coin: any) => (
            <div
              key={coin.name}
              className="flex flex-col gap-2 p-4 bg-[#131722] shadow-lg border border-[#2A2E39] rounded-xl hover:-translate-y-1 transition-transform cursor-pointer"
            >
              <img
                src={
              ...
              <AnimatePresence mode="popLayout">
                <motion.span
                  ...
                  initial={{ opacity: 0.5, y: -2 }}
                  animate={{ opacity: 1, y: 0 }}
                  className={`font-mono text-xs font-bold px-1 py-0.5 rounded w-fit ${coin.isUp ? "text-blue-400 bg-blue-500/100/10" : "text-red-400 bg-red-500/10"}`}
                >
                  {coin.price}
                </motion.span>
              </AnimatePresence>
              <span
                className={`font-mono text-[11px] font-bold ${coin.isUp ? "text-blue-400" : "text-red-400"}`}
              >
                {coin.change}
              </span>
            </div>
