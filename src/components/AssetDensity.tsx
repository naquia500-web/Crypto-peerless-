          ...
          <div key={idx} className="flex items-center gap-2 px-3 py-1.5 bg-[#131722] shadow-lg border border-[#2A2E39] rounded-full hover:bg-[#2A2E39] transition-colors cursor-pointer group">
            ...
            <AnimatePresence mode="popLayout">
              <motion.span 
                ...
                className={`text-[10px] font-mono font-bold px-1 rounded ${item.isUp ? 'text-blue-600 bg-blue-500/10' : 'text-red-500 bg-red-500/10'}`}
              >
                {item.price}
              </motion.span>
            </AnimatePresence>
            <span className={`text-[10px] font-mono font-bold ${item.isUp ? 'text-blue-600' : 'text-red-500'}`}>
              {item.change}
            </span>
          </div>
        ))}
