...
import { Calculator, Pickaxe, Scale, HandCoins, Play, Activity, ArrowRightLeft, DollarSign } from 'lucide-react';
...
export function ProCryptoTools() {
  ...
  const futuresPriceDiff = futuresPosition === 'long' 
    ? futuresExit - futuresEntry 
    : futuresEntry - futuresExit;
    
  const futuresPnl = futuresPriceDiff * futuresQuantity;
  const futuresRoe = futuresInitialMargin > 0 ? (futuresPnl / futuresInitialMargin) * 100 : 0;


  const mockDb: Record<string, any> = {
    'bitcoin': { cap: '$1.4T', tps: '7', consensus: 'PoW', cost: '$4.50' },
    'ethereum': { cap: '$340.5B', tps: '15-30', consensus: 'PoS', cost: '$2.50' },
    'solana': { cap: '$42.1B', tps: '65,000+', consensus: 'PoH/PoS', cost: '$0.00025' },
    'cardano': { cap: '$15B', tps: '250', consensus: 'dPoS', cost: '$0.10' },
  };

  const getStats = (name: string) => {
    const key = name.toLowerCase();
    if (key.includes('bitcoin') || key.includes('btc')) return mockDb['bitcoin'];
    if (key.includes('ethereum') || key.includes('eth')) return mockDb['ethereum'];
    if (key.includes('solana') || key.includes('sol')) return mockDb['solana'];
    if (key.includes('cardano') || key.includes('ada')) return mockDb['cardano'];
    // Default fallback
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-[#1E222D]">
        {activeTab === 'defi' && (
          <div className="max-w-md mx-auto bg-[#131722] p-6 rounded-xl border border-[#2A2E39] shadow-sm">
            <h3 className="font-bold text-white mb-4">DeFi Staking & Lending</h3>
            <div className="space-y-4">
              ...
              <div className="pt-4 border-t border-[#2A2E39] flex flex-col gap-2">
                <div className="flex justify-between items-center">
                  <span className="font-bold text-[#787B86] text-sm">Estimated Daily Rewards</span>
                  <span className="text-sm font-bold text-white">${defiDailyReward.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="font-bold text-[#787B86] text-sm">Estimated Yearly Rewards</span>
                  <span className="text-lg font-black text-green-500">${defiYearlyReward.toFixed(2)}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <h3 className="font-bold text-white mb-4">Mining Profitability Est.</h3>
             <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-[#787B86] uppercase">Power (W)</label>
                  <input 
                    type="number" 
                    value={minePower} 
                ...
                <div>
                  <label className="text-xs font-bold text-[#787B86] uppercase">Cost ($/kWh)</label>
                  <input 
                    type="number" 
                    step="0.01"
                    value={mineCost} 
              ...
              <div className="pt-4 border-t border-[#2A2E39] flex flex-col gap-2">
                <div className="flex justify-between items-center text-sm">
                  <span className="font-bold text-[#787B86]">Daily Revenue</span>
                  <span className="font-bold text-white">${mineDailyRevenue.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="font-bold text-[#787B86]">Daily Power Cost</span>
                  <span className="font-bold text-red-400">-${mineDailyCost.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center mt-2 pt-2 border-t border-dashed border-[#2A2E39]">
                  ...
                  <span className={`text-xl font-black ${mineDailyProfit > 0 ? 'text-green-500' : 'text-red-500'}`}>
                    {mineDailyProfit > 0 ? '+' : ''}${mineDailyProfit.toFixed(2)}
                  </span>
                </div>
              </div>
            </div>
            ...
            <table className="w-full text-sm">
              <tbody>
                {[
                  ...
                  { label: "Consensus", a: statsA.consensus, b: statsB.consensus },
                  { label: "Avg Tx Cost", a: statsA.cost, b: statsB.cost },
                ].map((row, i) => (
                  <tr key={i} className="border-b border-[#2A2E39] last:border-0 hover:bg-[#1E222D] transition-colors rounded-lg">
                    <td className="py-3 font-bold text-[#B2B5BE] w-[40%] text-center">{row.a}</td>
                    <td className="py-3 text-[9px] sm:text-[10px] font-black uppercase text-slate-500 text-center w-[20%] tracking-wider">{row.label}</td>
                    <td className="py-3 font-bold text-[#B2B5BE] w-[40%] text-center">{row.b}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {activeTab === 'tax' && (
          <div className="max-w-md mx-auto bg-[#131722] p-6 rounded-xl border border-[#2A2E39] shadow-sm">
            <div className="flex items-center gap-2 mb-4">
               ...
               <h3 className="font-bold text-white text-lg">Futures Estimator</h3>
            </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-[#787B86] uppercase">Entry Price</label>
                  <div className="relative mt-1">
                ...
                <div>
                  <label className="text-xs font-bold text-[#787B86] uppercase">Exit Price</label>
                  <div className="relative mt-1">
                    <DollarSign className="w-4 h-4 text-gray-500 absolute left-2 top-1/2 -translate-y-1/2" />
                    <input 
                      type="number" 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-[#787B86] uppercase">Quantity (BTC)</label>
                  <input 
                    type="number" 
                ...
                <div>
                  <label className="text-xs font-bold text-[#787B86] uppercase">Leverage (x)</label>
                  <input 
                    type="number" 
            ...
            <div className="mt-6 p-4 bg-[#1E222D] rounded-xl border border-[#2A2E39] flex flex-col gap-3">
               <div className="flex justify-between items-center text-sm">
                 ...
                 <span className="font-bold text-white">${futuresInitialMargin.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
               </div>
               <div className="flex justify-between items-center text-sm">
                 <span className="font-bold text-[#787B86]">Position Size</span>
                 <span className="font-bold text-white">${futuresPositionSize.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
               </div>
               <div className="h-px bg-[#2A2E39] my-1" />
               <div className="flex justify-between items-center text-sm">
                 <span className="font-bold text-[#787B86]">Estimated PNL</span>
                 <span className={`font-black ${futuresPnl >= 0 ? 'text-green-500' : 'text-red-500'}`}>
               ...
               <div className="flex justify-between items-center text-sm">
                 ...
                 <span className={`font-black ${futuresRoe >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                   {futuresRoe >= 0 ? '+' : ''}{futuresRoe.toFixed(2)}%
                 </span>
               </div>
            </div>
          </div>
          ...
          <div className="max-w-md mx-auto bg-[#131722] p-8 rounded-xl border border-[#2A2E39] shadow-sm text-center py-10 flex flex-col items-center">
            {!isSimActive ? (
              <>
                ...
                <h3 className="font-bold text-white mb-2 text-lg">Virtual Trading Simulator</h3>
              ...
              <div className="w-full text-left">
                <div className="flex justify-between items-center mb-6">
                   <h3 className="font-bold text-white text-lg">Simulator Active</h3>
                   <span className="px-2 py-1 bg-green-500/20 text-green-500 text-xs font-bold rounded">LIVE</span>
                </div>
                <div className="p-4 bg-[#1E222D] rounded-lg border border-[#2A2E39] mb-4">
                  <div className="text-xs text-gray-400 font-bold uppercase tracking-wider mb-1">Available Balance</div>
                  <div className="text-3xl font-black text-white">${simBalance.toLocaleString()}</div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <button 
