...
3.  **Explore Yield:** Your ETH could be staked to generate passive yield, compounding your holdings over time.`;
  ...
  return (
    <div className="bg-[#0B0E11] border border-white/10 rounded-2xl p-6 flex flex-col h-full relative overflow-hidden">
      ...
      <div className="flex justify-between items-start mb-6 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/50 font-mono mt-0.5">Secure Analysis & Diversification</p>
          </div>
        </div>
        ...
        {!isConnected ? (
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-xs font-bold uppercase tracking-widest text-white/80 transition-colors"
          >
      ...
      {!isConnected ? (
        <div className="flex-1 flex flex-col items-center justify-center p-8 border border-dashed border-white/10 rounded-xl bg-white/[0.01]">
          <Activity className="w-12 h-12 text-white/20 mb-4" />
          <p className="text-sm text-white/60 text-center max-w-sm">Connect your exchange or wallet to allow the AI Doctor to scan your holdings and provide personalized health checks.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 relative z-10">
          <div className="lg:col-span-1 flex flex-col gap-4">
            <div className="bg-black/40 border border-white/5 rounded-xl p-4">
              ...
              <div className="flex flex-col gap-2">
                {portfolio.map(asset => (
                  <div key={asset.asset} className="flex justify-between items-center text-sm border-b border-white/5 pb-2 last:border-0 last:pb-0">
                    <span className="font-bold text-white/80">{asset.asset}</span>
                    <span className="font-mono text-white/60">${asset.value.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="lg:col-span-2 min-h-[300px] border border-white/5 rounded-xl bg-black/40 p-6 flex flex-col">
            {isAnalyzing ? (
