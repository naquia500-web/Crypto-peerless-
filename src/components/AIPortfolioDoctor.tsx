...
export function AIPortfolioDoctor() {
  ...
  const [portfolio, setPortfolio] = useState<PortfolioData[]>([
    ...
    { asset: "ETH", amount: 4.2, value: 12400.2 },
    { asset: "SOL", amount: 150, value: 21000.0 },
    { asset: "DOGE", amount: 10000, value: 1500.0 },
  ]);
  ...
  const connectWallet = () => {
    setIsConnected(true);
    runAnalysis(); // Auto run analysis on first connect
  };
  ...
  const fetchLivePortfolioData = async () => {
    ...
    try {
      ...
      const sol = data.find((t: any) => t.symbol === "SOLUSDT");
      const doge = data.find((t: any) => t.symbol === "DOGEUSDT");

      if (btc && eth && sol && doge) {
        setPortfolio([
          { asset: "BTC", amount: 0.5, value: 0.5 * parseFloat(btc.lastPrice) },
          { asset: "ETH", amount: 4.2, value: 4.2 * parseFloat(eth.lastPrice) },
          { asset: "SOL", amount: 150, value: 150 * parseFloat(sol.lastPrice) },
          {
            asset: "DOGE",
            amount: 10000,
            value: 10000 * parseFloat(doge.lastPrice),
          },
        ]);
        runAnalysis(data);
      }
    } catch (e) {
      console.error("Live portfolio sync failed", e);
    }
  };
  ...
  useEffect(() => {
    if (isConnected) {
      ...
      const interval = setInterval(fetchLivePortfolioData, 3600000); // 1 hour update
      return () => clearInterval(interval);
    }
  }, [isConnected]);

  const runAnalysis = async (liveMarketData?: any) => {
    setIsAnalyzing(true);
    try {
      await new Promise((r) => setTimeout(r, 1500));

      const btcPercent = (
        ((portfolio.find((a) => a.asset === "BTC")?.value || 0) / totalValue) *
        100
      ).toFixed(1);
      const ethPercent = (
        ((portfolio.find((a) => a.asset === "ETH")?.value || 0) / totalValue) *
        100
      ).toFixed(1);

      let liveContext = "";
      if (liveMarketData) {
        const btcData = liveMarketData.find((t: any) => t.symbol === "BTCUSDT");
        const btcChange = parseFloat(btcData.priceChangePercent);
        liveContext = `\n\n#### ⚡ Live Market Context (Updated Real-time)\n*   **Market Trend:** BTC is currently ${btcChange >= 0 ? "up" : "down"} ${btcChange.toFixed(2)}% in the last 24h. ${btcChange >= 0 ? "Your portfolio is well-positioned for this bullish momentum." : "Consider structural stop-losses to protect downside risk."}`;
      }
...
*   **Top Heavy:** Your portfolio is heavily concentrated in Bitcoin (${btcPercent}%) and Ethereum (${ethPercent}%). This provides a strong, relatively stable foundation.
...
You lack exposure to emerging sectors such as Layer-2 scaling solutions, Real World Assets (RWA), or AI-focused tokens. ${liveContext}
    ...
    <div className="bg-[#131722] border border-[#2A2E39] rounded-2xl p-6 flex flex-col h-full relative overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-white tracking-tight flex items-center gap-2">
              AI Portfolio Doctor
              <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-teal-500/10 border border-teal-500/20 ml-2">
            ...
            <p className="text-xs text-[#787B86] font-mono mt-0.5">
              Secure Analysis & Diversification
            </p>
          </div>
        </div>
      ...
      {!isConnected ? (
        <div className="flex-1 flex flex-col items-center justify-center p-8 border border-dashed border-[#2A2E39] rounded-xl bg-[#1E222D]">
          ...
          <p className="text-sm text-[#787B86] text-center max-w-sm">
            Connect your exchange or wallet to allow the AI Doctor to scan your
            holdings and provide personalized health checks with live market
            data tracking.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 relative z-10">
          <div className="lg:col-span-1 flex flex-col gap-4">
            <div className="bg-[#1E222D] border border-[#2A2E39] rounded-xl p-4">
              <h3 className="text-[10px] uppercase font-bold tracking-widest text-white/40 mb-3">
                Current Holdings
              </h3>
              ...
              <div className="flex flex-col gap-2">
                {portfolio.map((asset) => (
                  <div
                    key={asset.asset}
                    className="flex justify-between items-center text-sm border-b border-[#2A2E39] pb-2 last:border-0 last:pb-0"
                  >
            ...
            {isAnalyzing ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-4 text-white/40">
                ...
                <div className="text-sm font-mono animate-pulse text-center">
                  Diagnosing Portfolio Risk Profile...
                </div>
              </div>
            ) : analysis ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center gap-4 opacity-30">
                ...
                <span className="text-sm font-mono text-center max-w-xs">
                  Run a health check to receive institutional-grade risk and
